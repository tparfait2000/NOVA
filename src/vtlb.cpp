...
//        Console::print("entry start: %llx  host: %lx", tlb->val, host);
        if(tlb->is_cow_pf(tlb->val, error)){
            return SUCCESS;
        }else{
...
//        Console::print("entry end: %llx host: %lx", tlb->val, host);
        Vtlb::set_cow_page_vmx(virt, tlb->val);
        return SUCCESS;
    }
}
...
bool Vtlb::is_cow_pf(uint64 &entry, mword err){
    if ((entry & TLB_COW) && (entry & TLB_U)) {
        if (!(entry & TLB_P) && (entry & TLB_COW_IO)) {
            Console::print("PTE_COW_IO");
            return true;
        } else if ((err & ERR_W) && !(entry & TLB_W)) {
//            Console::print("COW_FAULT entry: %llx", entry);
            entry |= TLB_W;
            entry &=~TLB_COW;
            return true;
        }
//        Console::print("NO COW1");
        return false;
    }
//    Console::print("entry : %llx", entry);
    return false;
}
